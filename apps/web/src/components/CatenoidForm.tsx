...
export function CatenoidForm() {
  ...
  const onSubmit = useCallback(
    (values: FormValues) => {
      ...
      setCatenoidInput({ ...values.a, z: 0 });
    },
    [setCatenoidInput, setQueryStatus]
  );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} onSubmitCapture={saveCheckpoint} className="space-y-4" noValidate>
      <div className="grid gap-3 sm:grid-cols-2">
        <Vec2Fields register={register} prefix="a" label="Waist parameter (a)" symbol={objectLabels.catenoidA} errors={errors.a} />
      </div>
      <p className="text-xs leading-5 text-slate-500">A larger a is a wider, flatter waist; a smaller a pinches the film in more sharply — the ring height is fixed.</p>
      <FormActions isSubmitting={isSubmitting} label="Recompute the film" />
    </form>
  );
}

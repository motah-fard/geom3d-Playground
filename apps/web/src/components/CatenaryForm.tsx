...
export function CatenaryForm() {
  ...
  const onSubmit = useCallback(
    (values: FormValues) => {
      ...
      setCatenaryInput({ ...values.a, z: 0 });
    },
    [setCatenaryInput, setQueryStatus]
  );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} onSubmitCapture={saveCheckpoint} className="space-y-4" noValidate>
      <div className="grid gap-3 sm:grid-cols-2">
        <Vec2Fields register={register} prefix="a" label="Catenary parameter (a)" symbol={objectLabels.catenaryA} errors={errors.a} />
      </div>
      <p className="text-xs leading-5 text-slate-500">A larger a is a tauter, flatter curve; a smaller a sags more steeply — the span is fixed.</p>

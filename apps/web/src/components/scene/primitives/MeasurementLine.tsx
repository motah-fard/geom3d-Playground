...
export function MeasurementLine({ start, end, label = "distance" }: { start: Vec3; end: Vec3; label?: string }) {
  ...
  return (
    <>
      <Line points={[toTuple(start), toTuple(end)]} color="#FF7AC8" lineWidth={2} dashed dashSize={0.18} gapSize={0.1} />
      <Html center position={midpoint} style={{ pointerEvents: "none" }}>
        <span className="whitespace-nowrap rounded-md border border-[#FF7AC8]/20 bg-slate-950/90 px-2 py-1 font-mono text-[10px] text-[#FF9FD6] shadow-xl">{label} {Number(distance.toFixed(precision))} {unit}</span>
      </Html>
    </>
  );
}
